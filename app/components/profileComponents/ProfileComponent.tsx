...
const ProfileComponent = () => {
  ...
  const onSubmitAdmineprofileupdate: SubmitHandler<IAdmineProfileForm> = async (data) => {
    try {
      const payload = userData ? { ...data, _id: userData._id } : data;
      const { status } = await axiosInstance.put("/users/update-profile", payload);
      if (status === 200) {
        ...
        Cookies.set("loggedInUser", JSON.stringify(updatedUserData), { expires: 7 });
        Cookies.set("First-name", JSON.stringify(data.first_name), { expires: 7 });
        Cookies.set("last-name", JSON.stringify(data.last_name), { expires: 7 });
        Cookies.set("email", JSON.stringify(data.email), { expires: 7 });
        ...
        <div className="flex flex-col items-center md:items-start md:w-1/3 gap-4 mb-8 md:mb-0">
          ...
          <div className="mt-2 text-center md:text-left flex flex-col space-y-6">
            <h1 className="text-2xl font-extrabold text-gray-800 mb-1">
              {userInfo ? `${userInfo.first_name} ${userInfo.last_name}` : "Admin User"}
            </h1>
